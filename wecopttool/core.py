...
_shm_sign = 1.0  # +/- 1.0: x(t) = Re{Xe^(+/-iωt)}
...
def time_mat(f0: float, nfreq: int, nsubsteps: int = 1) -> np.ndarray:
    ...
    time_mat[:, 2::2] = _shm_sign * -np.sin(wt)
    return time_mat


def derivative_mat(f0: float, nfreq: int) -> np.ndarray:
    block = lambda n: _shm_sign*np.array([[0, -1], [1, 0]]) * n*f0 * 2*np.pi
...
def _check_damping(bem_data, tol=1e-6) -> xr.Dataset:
    ...
    if dmin <= 0.0 + tol:
        ...
        # TODO: Should this be added only to the diagonal of 'friction'?
        #       Only to the diagonal terms that fail check?
        # TODO: check sign based on new convention... use`_shm_sign`
    return bem_data
...
def standard_forces(bem_data: xr.Dataset):
    ...
    position_transfer_functions['radiation'] = _shm_sign * 1j*w*(B + 1j*w*A)
    position_transfer_functions['hydrostatics'] = (w*0+1) * K + 0j # TODO: better way of repeating K over omega dimension?
    position_transfer_functions['friction'] = _shm_sign * 1j*w*Bf
    ...
    for name, value in position_transfer_functions.items():
        linear_force_functions[name] = f_from_imp(mimo_transfer_mat(value, ndof))
...
def mimo_transfer_mat(imp: np.ndarray, ndof:int) -> np.ndarray:
    ...
    block = lambda re, im: np.array([[re, _shm_sign*im], [_shm_sign*-im, re]])
...
def write_netcdf(fpath: str | Path, data: xr.Dataset) -> None:
    ...
    file.
    """
    cpy.io.xarray.separate_complex_values(data).write_netcdf(fpath)
