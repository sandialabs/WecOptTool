...
def run_bem(
    ...
    if not hasattr(wec_im, 'inertia_matrix'):
        _log.warning('FloatingBody has no inertia_matrix field, using ' + 
                     'neutral buoyancy assumption to auto-populate')
    ...
    if not hasattr(wec_im, 'hydrostatic_stiffness'):
        _log.warning('FloatingBody has no hydrostatic_stiffness field. ' + 
                     'If the FloatingBody mass is predefined, it will be ' + 
                     'used for calculting the hydrostatic stiffness here. ' + 
