...
class WEC:
    ...
    def solve(self, waves: xr.Dataset,
        ...
        for icons in constraints:
            fun = icons['fun']
            icons['jac'] = jacobian(fun)
            # hess = hessian(fun)
            # icons['hess'] = lambda x, v: hess(x)

        res = minimize(fun=obj_fun_scaled,
                       x0=x0,
                       jac=grad(obj_fun_scaled),
                    #    hess=hessian(obj_fun_scaled),
