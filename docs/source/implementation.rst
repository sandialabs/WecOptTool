...
The corresponding time vector for the chosen frequency array :math:`f = [0, f_1, 2 f_2, ..., n f_1]` is :math:`t = [0, \Delta t, 2 \Delta t, ..., (2n-1) \Delta t]`, where :math:`\Delta t = t_f/(2n)` and the repeat period is :math:`t_f=2n\Delta t`.
...
In general, a state :math:`x` is represented as the mean (DC) component followed by the real and imaginary components of the Fourier coefficients as :math:`x=[X_0, Re(X1), Im(X1), ..., Re(Xn), Im(Xn)]`.
...
Alternatively, we sometimes need to represent the state as a complex vector :math:`x=[X_0, X_1, ..., X_n]`.
